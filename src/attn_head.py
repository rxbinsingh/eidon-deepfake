...
class DeepfakeHead(nn.Module):
    ...
    def __init__(self, dim=EMBED_DIM, heads=4, hidden=256, dropout=0.3):
        super().__init__()
        self.face_norm = nn.LayerNorm(dim)
        self.full_norm = nn.LayerNorm(dim)
        self.face_pool = AttnPool(dim, heads, dropout=0.1)
        self.full_pool = AttnPool(dim, heads, dropout=0.1)
        self.no_face = nn.Parameter(torch.randn(dim) * 0.02)
    ...
    def forward(self, face, full, face_mask, full_mask, face_valid):
        ...
        f = self.face_pool(self.face_norm(face), face_mask)
        g = self.full_pool(self.full_norm(full), full_mask)
...
class EmbeddingBank:
    ...
    def __init__(self, stems, embed_cache_dir, device="cuda"):
